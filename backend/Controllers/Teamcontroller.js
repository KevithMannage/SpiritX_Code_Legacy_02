...
//         res.status(500).json({ error: 'Failed to get player count', details: err.message });
//     }
// };
...
//     console.log('Request received:', { User_ID, Player_ID });
...
//     try {
//         const result = await addPlayerToTeam(User_ID, Player_ID);
...
//         const result = await removePlayerFromTeam(userId, playerId);
...
//         res.status(500).json({ error: 'Failed to remove player', details: err.message });
//     }
// };
import {
    countPlayersInTeam,
  ...
  // Get the list of players in a user's team
